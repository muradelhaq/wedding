...
<section class="py-12 px-6 max-w-4xl mx-auto">
    <div class="text-center mb-12" data-reveal="fade-down">
        <p class="text-xs uppercase tracking-[0.3em] text-[#57795c] font-serif font-semibold mb-1" x-text="t('bismillah_sub')">Maha Suci Allah yang Menciptakan Pasangan</p>
        <h2 class="font-serif text-3xl sm:text-4xl font-bold text-[#233327] mb-3" x-text="t('couple_title')">Kedua Mempelai</h2>
        <div class="w-16 h-1 bg-gradient-to-r from-transparent via-[#6F9575] to-transparent mx-auto rounded-full"></div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
        <!-- Groom Card -->
        <div class="relative bg-gradient-to-b from-white to-[#F4F7F4] border-2 border-[#D5E2D7] rounded-3xl p-8 text-center shadow-[0_10px_35px_rgba(111,149,117,0.12)] transition duration-300 hover:shadow-xl hover:border-[#6F9575] overflow-hidden" 
            ...
            <p class="text-xs font-semibold uppercase tracking-widest text-[#57795c] mb-4 font-serif" 
               data-reveal="fade-up" data-reveal-delay="350"
               x-text="t('groom_label')">
                — Mempelai Pria —
            </p>

            <div class="text-sm text-[#526356] space-y-1 mb-6" data-reveal="fade-up" data-reveal-delay="400">
                <p class="text-xs" x-text="t('groom_son_of')">Putra tercinta dari:</p>
                ...
                <p class="text-xs text-[#314736] font-medium mt-2 bg-[#6F9575]/15 py-1 px-3 rounded-full inline-block" x-text="t('groom_origin')">Keluarga {{ $settings['couple']['groom_origin'] ?? 'Istanbul, Turki' }}</p>
            </div>

            @if(!empty($settings['couple']['groom_instagram']))
                @php $groomIg = ltrim($settings['couple']['groom_instagram'], '@'); @endphp
                <a href="https://instagram.com/{{ $groomIg }}" target="_blank" rel="noopener noreferrer" 
        ...
        <div class="relative bg-gradient-to-b from-white to-[#F4F7F4] border-2 border-[#D5E2D7] rounded-3xl p-8 text-center shadow-[0_10px_35px_rgba(111,149,117,0.12)] transition duration-300 hover:shadow-xl hover:border-[#6F9575] overflow-hidden"
            ...
            <div class="text-sm text-[#526356] space-y-1 mb-6" data-reveal="fade-up" data-reveal-delay="450">
                ...
                <p class="text-xs text-[#314736] font-medium mt-2 bg-[#6F9575]/15 py-1 px-3 rounded-full inline-block" x-text="t('bride_origin')">Keluarga {{ $settings['couple']['bride_origin'] ?? 'Garut, Indonesia' }}</p>
            </div>

            @if(!empty($settings['couple']['bride_instagram']))
                @php $brideIg = ltrim($settings['couple']['bride_instagram'], '@'); @endphp
                <a href="https://instagram.com/{{ $brideIg }}" target="_blank" rel="noopener noreferrer" 
